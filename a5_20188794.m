...
%         none
    ...
    % To calculate accuracy : (TP + TN)/ all predictions
    % Finding TP + TN first
    
    % find the quantization vectors for ann and log approach
    qvec_ann = z_ann >= 0;
    qvec_log = z_log >= 0;
    
    % find the differences between quantization and y labels
    % and store them in error vectors
    err_ann = yvec - qvec_ann;
    err_log = yvec - qvec_log;
    
    % TP + TN is all the correct predictions made by the hyperplanes
    % thus TP + TN can be calculated as all the time the error vector 
    % has the value 0 
    tptn_ann = 0;
    tptn_log = 0;
    
    for idx = 1: size(yvec,1)
        if err_ann(idx) == 0
            tptn_ann = tptn_ann + 1;
        end 
        if err_log(idx) == 0
    ...
    acc_ann = tptn_ann / size(yvec, 1);
    acc_log = tptn_log / size(yvec, 1);
    disp('Perceptron hyperplace accuracy')
    disp(acc_ann)
    disp('Logistic Regression hyperplane accuracy')
    disp(acc_log)
    ...
    gscatter(Xmat(:, 1), Xmat(:, 2), yvec, 'rb');
    plotline(v_log, 'm');
    title('Hyperplane for Logisitic regression')
    xlabel('Z1')
    ylabel('Z2')
...
%         none
    ...
    one = ones(size(Eval, 1), 1);
    
    % sorting to get the first 2 eigenvectors for 2D projection
    [sort_Eval, Eind] = sort(Eval*one, 'descend');
    sort_Evec = Evec(:, Eind);
    ...
    figure(5);
    gscatter(sort_Evec(:, 1), sort_Evec(:, 2),yvec, 'rb')
    title('Kernel PCA on yvec')
    xlabel('Z1')
    ylabel('Z2')

    figure(6);
    gscatter(sort_Evec(:, 1), sort_Evec(:, 2),yk2, 'mc')
    title('Kernel PCA on yk2')
    xlabel('Z1')
    ylabel('Z2')
...
%         K       NxN Gram matrix
    ...
    for idx = 1:m
        for jdx = 1:m
            Kmat(idx, jdx) = exp(-1/sigma2*norm(Xmat(idx,:) - Xmat(jdx,:))^2);
        end
    end
end
